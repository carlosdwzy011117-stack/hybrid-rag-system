...
- **Streamlit 可交互 demo**：实时切换 BM25 / Dense / Weighted 三种检索器，肉眼对比同一 query 在不同方法下的检索差异与 LLM 答案。
...
<p align="center">
  <img src="docs/demo_overview.png" alt="Demo 主界面" width="80%"/>
</p>

**同一 query，不同 retriever 揭示不同的相关性定义：**

<table>
<tr>
<td><img src="docs/demo_bm25.png" alt="BM25 检索结果" width="100%"/></td>
<td><img src="docs/demo_weighted.png" alt="Weighted 检索结果" width="100%"/></td>
</tr>
<tr>
<td align="center"><b>BM25（稀疏）</b> —— 基于词项重叠</td>
<td align="center"><b>Weighted Fusion（α=0.3）</b> —— 语义 + 关键词融合</td>
</tr>
</table>

在 query `"0-dimensional biomaterials show inductive properties"` 上：BM25 抓 "stem cell / epithelial / EMT" 等关键词重叠的文档；Weighted Fusion 抓"生物材料 / 微结构 / 干细胞分化"语义簇下的文档。两种检索器在 top-5 都未命中 gold doc `31715818`（gold 在 BM25 top-500 之外、在 Weighted 排第 9），LLM 据此诚实回答"文档中无相关信息" —— 演示了 RAG faithfulness 防幻觉的设计意图。
...
**Streamlit demo 的 cache_resource 模式**。BM25 索引、Dense 索引（BGE 编码 5K 文档）、Generator 实例都封装到 `@st.cache_resource` 装饰的加载函数里，确保首次启动跑一次、之后用户每次操作都直接复用缓存。如果不缓存，每次切换 retriever / 改 query 都会重建 Dense 索引、等待 1-3 分钟，demo 无法实际使用。
...
**Phase 5 — 产品化（✅ 完成）**
- Streamlit demo 支持 retriever 实时切换
- 端到端可演示：query → 检索 → LLM 生成
- README 含 demo 截图与对比分析
